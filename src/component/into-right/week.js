...
function Week(props){
	...
	const RenderItem = (props) => {
		...
		const classList = classActive +" week-top-child box"

		return <div onClick={event => handleItemClick(event, props.index)}
                    className={classList}
                ...
                >
                    ...
                    <div className="content ">
                        <img src={require('./imgnot/img9.png')}  alt="imgnot" className="imgnot"/>
	...
	const listItems = weekWeather.map((item, index) => (
		<RenderItem selected={(index == daySelected)?"bg-info":""} 
                    key={index.toString()} 
                    index={index} itemData={item} 
        />
	));
